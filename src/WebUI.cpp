...
#define FFT_BINS 256
extern uint8_t fftMagnitudes[];
extern volatile bool fftDataReady;
...
static String profileName(uint16_t buf) {
    ...
    if (buf <= 3072) return F("BirdNET-Go Default (~64 ms at 48 kHz, recommended)");
...
static String htmlIndex() {
    ...
    h += F(
        ...
        "<tr><td class='k' id='t_hostname'>Hostname</td><td class='v'><div class='field'><input id='in_hostname' type='text' maxlength='32' style='width:140px'><span class='unit'>.local</span><button onclick=\"setHostname()\">Set</button></div><div class='muted' style='font-size:11px;margin-top:3px'>WiFi DHCP name takes effect after reboot</div></td></tr>"
        ...
        "<div class='card'><h2>Spectrum</h2>"
        "<canvas id='wf' width='256' height='128' style='width:100%;image-rendering:pixelated;display:block;border-radius:3px'></canvas>"
        "<div style='display:flex;justify-content:space-between;font-size:10px;color:#666;margin-top:2px;padding:0 2px'><span>0</span><span>6k</span><span>12k</span><span>18k</span><span>24k Hz</span></div>"
        "</div>"
...
"function applyLang(){const L=T[lang]; const st=(id,t)=>{const e=$(id); if(e) e.textContent=t}; const help=(k)=>{const b=L[k]||''; return b}; st('t_title',L.title); st('t_status',L.status); st('t_ip',L.ip); st('t_wifi_rssi',L.wifi_rssi); st('t_wifi_tx',L.wifi_tx); st('t_heap',L.heap); st('t_uptime',L.uptime); st('t_rtsp_server',L.rtsp_server); st('t_client',L.client); st('t_streaming',L.streaming); st('t_pkt_rate',L.pkt_rate); st('t_last_connect',L.last_connect); st('t_last_play',L.last_play); st('t_audio',L.audio); st('t_rate',L.rate); st('t_gain',L.gain); st('t_buf',L.buf); st('t_latency',L.latency); st('t_level',L.level); st('t_profile',L.profile); st('t_perf',L.perf); st('t_auto',L.auto); st('t_wifi',L.wifi); st('t_wifi_tx2',L.wifi_tx2); st('t_thermal',L.thermal); st('t_therm_protect',L.therm_protect); st('t_therm_limit',L.therm_limit); st('t_therm_status',L.therm_status); st('t_therm_now',L.therm_now); st('t_therm_max',L.therm_max); st('t_therm_cpu',L.therm_cpu); st('t_therm_last',L.therm_last); st('t_logs',L.logs); st('b_srv_on',L.bsrvon); st('b_srv_off',L.bsrvoff); st('b_reset',L.breset); st('b_reboot',L.breboot); st('b_defaults',L.bdefaults); st('t_advanced_settings',L.advanced_settings); st('t_shift',L.shift); st('t_thr',L.thr); st('t_chk',L.chk); st('t_thr_mode',L.thr_mode); st('t_sched',L.sched); st('t_hours',L.hours); st('t_cpu',L.cpu); const hm=(id,k)=>{const e=$(id); if(e) e.setAttribute('title',help(k))}; hm('h_rate','help_rate'); hm('h_gain','help_gain'); hm('h_hpf','help_hpf'); hm('h_hpf_cut','help_hpf_cut'); hm('h_buf','help_buf'); hm('h_auto','help_auto'); hm('h_tx','help_tx'); hm('h_thr','help_thr'); hm('h_chk','help_chk'); hm('h_shift','help_shift'); hm('h_sched','help_sched'); hm('h_hours','help_hours'); hm('h_cpu','help_cpu'); hm('h_thr_mode','help_thr_mode'); hm('h_level','help_level'); hm('h_therm_protect','help_therm_protect'); hm('h_therm_limit','help_therm_limit'); st('btn_rate_set',L.set); st('btn_gain_set',L.set); st('btn_buf_set',L.set); st('btn_auto_set',L.set); st('btn_thrmode_set',L.set); st('btn_thr_set',L.set); st('btn_sched_set',L.set); st('btn_hours_set',L.set); st('btn_shift_set',L.set); st('btn_chk_set',L.set); st('btn_tx_set',L.set); st('btn_cpu_set',L.set); st('btn_oh_enable',L.set); st('btn_oh_limit',L.set); const sht=(id,k)=>{const e=$(id); if(e) e.textContent=help(k)}; sht('txt_rate_hint','help_rate'); sht('txt_gain_hint','help_gain'); sht('txt_hpf_hint','help_hpf'); sht('txt_hpf_cut_hint','help_hpf_cut'); sht('txt_buf_hint','help_buf'); sht('txt_auto_hint','help_auto'); sht('txt_thr_hint','help_thr'); sht('txt_thr_mode_hint','help_thr_mode'); sht('txt_sched_hint','help_sched'); sht('txt_hours_hint','help_hours'); sht('txt_shift_hint','help_shift'); sht('txt_chk_hint','help_chk'); sht('txt_tx_hint','help_tx'); sht('txt_cpu_hint','help_cpu'); sht('txt_level_hint','help_level'); sht('txt_therm_hint_protect','help_therm_protect'); sht('txt_therm_hint_limit','help_therm_limit'); st('t_hpf',L.hpf); st('t_hpf_cut',L.hpf_cut); st('t_agc',L.agc); hm('h_agc','help_agc'); st('btn_agc_set',L.set); sht('txt_agc_hint','help_agc'); st('t_led',L.led); hm('h_led','help_led'); st('btn_led_set',L.set); sht('txt_led_hint','help_led'); document.title=L.title;}"
        "function profileText(buf){const L=T[lang]; buf=parseInt(buf,10)||0; if(buf<=256) return L.profile_ultra; if(buf<=512) return L.profile_balanced; if(buf<=1024) return L.profile_stable; if(buf<=3072) return L.profile_birdnet; return L.profile_high;}"
        ...
        "function setHostname(){ const v=($('in_hostname')||{}).value||''; if(v) setv('hostname',v.toLowerCase().replace(/[^a-z0-9-]/g,'')); }"
...
"let wfCtx=null,wfW=256,wfH=128;"
"function fftRGB(v){if(v<64)return[0,0,v*3];if(v<128)return[0,(v-64)*4,192-(v-64)*2];if(v<192)return[(v-128)*4,255,0];return[255,255-(v-192)*4,0];}"
"function drawDbMeter(pct,db){const c=$('dbm');if(!c)return;const x=c.getContext('2d'),W=c.width,H=c.height;x.fillStyle='#1c1c1c';x.fillRect(0,0,W,H);const bw=Math.round(Math.max(0,Math.min(100,pct))/100*W);const g=x.createLinearGradient(0,0,W,0);g.addColorStop(0,'#1a8a1a');g.addColorStop(0.72,'#8a8a00');g.addColorStop(0.88,'#cc5500');g.addColorStop(1,'#cc1111');x.fillStyle=g;x.fillRect(0,0,bw,H);x.fillStyle='rgba(255,255,255,0.75)';x.font='10px monospace';x.fillText(db.toFixed(1)+' dBFS',3,H-3);}"
"function initWaterfall(){const c=$('wf');if(!c)return;wfCtx=c.getContext('2d');wfW=c.width;wfH=c.height;wfCtx.fillStyle='#000';wfCtx.fillRect(0,0,wfW,wfH);}"
"function drawWaterfall(bins){if(!wfCtx)return;wfCtx.drawImage(wfCtx.canvas,-1,0);const col=wfCtx.createImageData(1,wfH);const d=col.data;for(let i=0;i<wfH;i++){const bi=(wfH-1-i)*2;const v=Math.max(bins[bi]||0,bins[bi+1]||0);const[r,g,b]=fftRGB(v);const p=i*4;d[p]=r;d[p+1]=g;d[p+2]=b;d[p+3]=255;}wfCtx.putImageData(col,wfW-1,0);}"
"function loadFFT(){fetch('/api/fft',{cache:'no-store'}).then(r=>r.json()).then(j=>{if(j.ready)drawWaterfall(j.bins);}).catch(()=>{});}"
"setInterval(loadAll,3000); setInterval(loadFFT,200);"
...
"trackEdit($('in_hostname'),'hostname'); trackEdit($('sel_led'),'led_mode'); trackEdit($('in_auto'),'auto_recovery'); trackEdit($('in_thr_mode'),'thr_mode'); trackEdit($('in_sched'),'sched_reset'); trackEdit($('sel_buf'),'buffer'); trackEdit($('sel_tx'),'wifi_tx'); trackEdit($('sel_hp'),'hp_enable'); trackEdit($('sel_agc'),'agc_enable'); trackEdit($('sel_cpu'),'cpu_freq'); trackEdit($('sel_oh_enable'),'oh_enable'); trackEdit($('sel_oh_limit'),'oh_limit');"
...
static void httpStatus() {
    ...
    json += "\"hostname\":\"" + jsonEscape(currentHostname) + "\"";
    json += "}";
    apiSendJSON(json);
}
...
static void httpFFT() {
    uint8_t snap[FFT_BINS];
    bool rdy;
    portENTER_CRITICAL(&fftMux);
    rdy = fftDataReady;
    if (rdy) memcpy(snap, fftMagnitudes, FFT_BINS);
    portEXIT_CRITICAL(&fftMux);
    if (!rdy) { apiSendJSON(F("{\"ready\":false}")); return; }
    String json;
    json.reserve(4 + FFT_BINS * 4);
    json = F("{\"ready\":true,\"bins\":[");
...
static void httpSet() {
    ...
    // i2sShiftBits removed - fixed at 0 for PDM microphones
        ...
        v.toLowerCase();
        bool ok = v.length() >= 1 && v.length() <= 32 && v[0] != '-' && v[v.length()-1] != '-';
        for (size_t i = 0; ok && i < (size_t)v.length(); i++) {
            char c = v[i];
            if (!((c>='a'&&c<='z')||(c>='0'&&c<='9')||c=='-')) ok = false;
        }
        if (ok) { currentHostname = v; saveAudioSettings(); applyHostname(); }
    }
