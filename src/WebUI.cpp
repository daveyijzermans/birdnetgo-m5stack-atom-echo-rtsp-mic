...
"function drawDbMeter(pct,db){const c=$('dbm');if(!c)return;const x=c.getContext('2d'),W=c.width,H=c.height;x.fillStyle='#1c1c1c';x.fillRect(0,0,W,H);const bw=Math.round(Math.max(0,Math.min(100,pct))/100*W);const g=x.createLinearGradient(0,0,W,0);g.addColorStop(0,'#1a8a1a');g.addColorStop(0.72,'#8a8a00');g.addColorStop(0.88,'#cc5500');g.addColorStop(1,'#cc1111');x.fillStyle=g;x.fillRect(0,0,bw,H);}"
